...
public class Day09Solution {
  ...
  public static List<Command> parseCommands(List<String> input) {
    return input.stream().map(Day09Solution::toCommand).collect(Collectors.toList());
  }
  ...
  /**
   * Imagine a Cartesian reference like this: (0,0) ------------------------------------------> x |
   * | V y Going up reduces the y coordinate, going down increases it. Going left reduce the x
   * coordinate, going right increases it. Both head and tail start at (0,0).
   */
